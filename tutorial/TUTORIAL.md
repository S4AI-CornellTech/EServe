...
Prereq: an EServe Python env with `server_carbon` + its `act_core` dependency. Standalone, point the
runner at it with `PYTHON=/path/to/venv/bin/python ./tutorial.sh …` (or activate it so `python3` works).
From the **full-stack-carbon** suite, `make setup` builds it and `make tutorial-eserve` runs these. Paths
below are relative to `EServe/tutorial/`.
...
`suggested_psu`. The bundled set is `../config/gpuconfigs.json` (use by `--gpu NAME`); the editable
copies for this tutorial live in `exercises/` (use by `--gpu-file …`, so the bundled config stays clean).
...
```
Now edit a *local* copy (`exercises/gpu_l4.json`) — the bundled config stays untouched — change one
field, re-run:
...
of server you learned to model in **ACT's tutorial** — here it dominates the GPU box.
...
`../config/gpuconfigs.json`, run `./tutorial.sh --gpu MyNewGPU`, then revert with
`git checkout ../config/gpuconfigs.json`. (The core exercises use local `exercises/*.json` precisely so
you never have to touch the bundled config.)
